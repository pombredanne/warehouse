...
class Version(object):
    ...
    def _parse(self, version):
        ...
        def _parse_numerical(numerical):
            ...
            def cast(number):
                if len(number) > 1 and number.startswith("0"):
                    raise ValueError("Cannot have leading zero in a version "
                        "number segment: '{number}' in '{version}'".format(
                            number=number,
                            version=version,
                        ))
                return int(number)
        ...
        if not match:
            raise ValueError(
                    "Invalid version '{version}'".format(version=version))
        ...
        if prerel is not None:
            parts += [tuple(
                    [prerel] + _parse_numerical(groups.get("prerelversion")))]
        else:
            parts += [("z",)]
        ...
        if parts[0][0] > 1980:
            raise ValueError("Huge major version number '{major}' in "
                    "'{version}', which might cause future problems".format(
                        major=parts[0][0],
                        version=version,
                ))
...
class VersionPredicate(object):
    ...
    def __init__(self, predicate):
        ...
        if match is None:
            raise ValueError(
                    "Bad predicate '{predicate}'".format(predicate=predicate),
                )
    ...
    def __eq__(self, other):
        if not isinstance(other, VersionPredicate):
            raise TypeError("Cannot compare {left} and {right}".format(
    ...
    def match(self, version):
        ...
        return all([self._operators[op](version, predicate)
                        for op, predicate in self.predicates])
