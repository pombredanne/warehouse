...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Download', fields ['date', 'project', 'version', 'user_agent', 'filename']
        db.delete_unique('warehouse_download', ['date', 'project', 'version', 'user_agent_id', 'filename'])

        # Adding field 'Download.label'
        db.add_column('warehouse_download', 'label',
                      self.gf('django.db.models.fields.CharField')(default='PyPI', max_length=25),
                      keep_default=False)

        # Adding unique constraint on 'Download', fields ['filename', 'project', 'version', 'user_agent', 'date', 'label']
        db.create_unique('warehouse_download', ['filename', 'project', 'version', 'user_agent_id', 'date', 'label'])


    def backwards(self, orm):
        # Removing unique constraint on 'Download', fields ['filename', 'project', 'version', 'user_agent', 'date', 'label']
        db.delete_unique('warehouse_download', ['filename', 'project', 'version', 'user_agent_id', 'date', 'label'])

        # Deleting field 'Download.label'
        db.delete_column('warehouse_download', 'label')

        # Adding unique constraint on 'Download', fields ['date', 'project', 'version', 'user_agent', 'filename']
        db.create_unique('warehouse_download', ['date', 'project', 'version', 'user_agent_id', 'filename'])
    ...
    models = {
        ...
        'warehouse.download': {
            'Meta': {'unique_together': "(('label', 'date', 'project', 'version', 'filename', 'user_agent'),)", 'object_name': 'Download'},
